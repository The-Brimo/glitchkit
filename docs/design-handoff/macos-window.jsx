...
// ─────────────────────────────────────────────────────────────
function MacGlass({ children, radius = 296, dark = false, fill = false, style = {} }) {
  return (
    <div style={{
      position: 'relative', borderRadius: radius,
      ...(fill ? { height: '100%', boxSizing: 'border-box', display: 'flex', flexDirection: 'column' } : null),
      ...style,
    }}>
      <div style={{
      ...
      <div style={{
        ...
        ...(fill ? { flex: 1, minHeight: 0, overflow: 'auto' } : null),
      }}>{children}</div>
    </div>
  );
}
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        color: 'rgba(255,255,255,0.9)', whiteSpace: 'nowrap', paddingLeft: 8,
      }}>{title}</div>
      <div style={{ flex: 1 }} />
      {/* single action */}
      <MacGlass>
        <div style={{
          ...
          <div style={{ width: 14, height: 14, borderRadius: '50%', background: '#e5e5ea', opacity: 0.5 }} />
        </div>
      </MacGlass>
      {/* search */}
      <MacGlass>
        <div style={{
          ...
          <svg width="13" height="13" viewBox="0 0 13 13" fill="none">
            <circle cx="5.5" cy="5.5" r="4" stroke="rgba(255,255,255,0.6)" strokeWidth="1.5"/>
            <path d="M8.5 8.5l3 3" stroke="rgba(255,255,255,0.6)" strokeWidth="1.5" strokeLinecap="round"/>
          </svg>
          <span style={{
            fontFamily: MAC_FONT, fontSize: 13, fontWeight: 500, color: 'rgba(255,255,255,0.55)',
          }}>Search</span>
        </div>
      </MacGlass>
    </div>
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        background: selected ? '#0a84ff' : 'rgba(255,255,255,0.5)',
        opacity: selected ? 1 : 0.5, flexShrink: 0, position: 'relative',
      }} />
      <span style={{ color: 'rgba(255,255,255,0.9)', position: 'relative' }}>{label}</span>
    </div>
  );
}
...
function MacSidebar({ children }) {
  return (
    <div style={{
      width: 220, height: '100%', padding: 8, flexShrink: 0, boxSizing: 'border-box',
      position: 'relative', display: 'flex', flexDirection: 'column',
    }}>
      {/* glass panel */}
      <div style={{
        position: 'absolute', inset: 8, borderRadius: 18,
        background: 'rgba(48,52,60,0.62)',
